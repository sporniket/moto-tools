...
class NameOfFile:
    def __init__(self, name:bytearray, suffix:bytearray):
        self._name = bytearray([0x20,0x20,0x20,0x20,0x20,0x20,0x20,0x20,0x20,0x20,0x20,0x20,0x20,0x20,0x20,0x20])
...
class CatalogEntry:
    def __init__(self, name:NameOfFile, typeOfFile:TypeOfFile, typeOfData:TypeOfData, firstBlock:int, blockchain:List[int], usageOfLastBlock:int, usageOfLastSector:int):
        self.name = name
    ...
    def line_catalogue(self) -> str:
        ...
        typeOfData = TYPE_OF_DATA_AS_CATALOG_STRING_WHEN_BASIC[self.typeOfData.value] if self.typeOfData == TypeOfData.BASIC_PROGRAM else TYPE_OF_DATA_AS_CATALOG_STRING[self.typeOfData.value]
        numberOfBlocks = len(self.blockchain)
        sizeOfFile = (numberOfBlocks - 1) * 2048 + 256 * (self.usageOfLastBlock - 1) + self.usageOfLastSector
...
class BlocAllocation:
    def __init__(self, id:int, status:int = BlockStatus.FREE.value):
        if id < BlockStatus.MIN_NEXT.value or id >= BlockStatus.MAX_NEXT.value:
            raise ValueError(f"id should be between {BlockStatus.MIN_NEXT.value} included and {BlockStatus.MAX_NEXT.value} excluded ; got {id}.")
        if status < BlockStatus.MIN_NEXT.value or (status >= BlockStatus.MAX_NEXT.value and status < BlockStatus.MIN_LAST.value) or (status >= BlockStatus.MAX_LAST and status < BlockStatus.RESERVED) or (status > BlockStatus.FREE.value) :
            raise ValueError(f"status should be between {BlockStatus.MIN_NEXT.value} included and {BlockStatus.MAX_NEXT.value} excluded, or between {BlockStatus.MIN_LAST.value} included and {BlockStatus.MAX_LAST.value} excluded, or {BlockStatus.RESERVED.value} or {BlockStatus.FREE.value} ; got {status}.")
        self.id = id
        self.status = status
        self.free = status == BlockStatus.FREE.value
        self.isLastBlock = status > BlockStatus.LAST_BLOCK.value and status < BlockStatus.MAX_LAST.value
        self.usage = 8 if status < BlockStatus.MAX_NEXT else status - BlockStatus.LAST_BLOCK.value if self.isLastBlock else 0
...
def extractBlockAllocationTableFromSector(data:bytearray or bytes) -> List[BlocAllocation]:
    usefullData = data[1:81]
    return [BlocAllocation(i, data[i]) for i in range(0,80)]

def extractCatalogEntriesFromSector(data:bytearray or bytes, bat:List[BlocAllocation]) -> List[CatalogEntry]:
    usefullData = data[0:256]
    result = []
    for i in range(0,256,32):
        entryData = usefullData[i:i+16] # the last 16 bytes of a 32-bytes long slice are unused
        if entryData[0] == 0xff:
            continue
        ...
        result += [
            CatalogEntry(NameOfFile(entryData[0:8],entryData[8:11]),TypeOfFile.fromInt(entryData[11]), TypeOfData.fromInt(entryData[12]), firstBlock, blockchain, entry[14]*256 + entry[15])
        ]
    return result

def extractCatalogFromTrack(data:bytearray or bytes, typeOfDiskImage:TypeOfDiskImage = EMULATOR_FLOPPY_IMAGE):
    intervalOfSlice = typeOfDiskImage.sizeOfSector()
    # extract BAT
    bat = extractBlockAllocationTableFromSector(data[intervalOfSlice:intervalOfSlice+256])
    
    # extract catalog entries.
    catalog = []
    for i in range(2,16):
        startOfSector = i*intervalOfSlice
        catalog += extractCatalogEntriesFromSector(data[startOfSector:startOfSector+256], bat)
...
class DiskSector:
    ...
    SIZE_OF_SECTOR_SDDRIVE = 512 # original sector filled with padding value to match SD Card sector size
    ...
    SDDRIVE_PADDING_DD = bytes([FILLER_SDDRIVE for i in range (SIZE_OF_SECTOR_SDDRIVE - SIZE_OF_SECTOR_DD)]) # padding to add after sector data
    ...
    def __init__(self, rawData:bytearray or bytes = bytes(), *, typeOfDiskImage:TypeOfDiskImage = TypeOfDiskImage.EMULATOR_FLOPPY_IMAGE):
        # always use self._size for the day there is simple density.
        self._size = SIZE_OF_SECTOR_DD
        self._typeOfDiskImage = typeOfDiskImage

        if typeOfDiskImage == TypeOfDiskImage.EMULATOR_FLOPPY_IMAGE :
            self._outputSize = SIZE_OF_SECTOR_DD
            self._padding = None
        else:
            self._outputSize = SIZE_OF_SECTOR_SDDRIVE
            self._padding = SDDRIVE_PADDING_DD

        # Only takes care of the floppy sector, without filling data for sddrive.
        if len(rawData) <self._size:
            self._data = bytearray([FILLER_FDDRIVE for i in range(self._size)])
            self._data[:len(rawData)] = rawData
        else:
            self._data = bytearray(rawData[:self._size])

    @property
    def data(self)->bytes:
        return bytes(self._data) if self.typeOfDiskImage == TypeOfDiskImage.EMULATOR_FLOPPY_IMAGE else bytes(self._data + self._padding)
...
class DiskTrack:
    ...
    def __init__(self, rawData:bytearray or bytes = bytes(), typeOfDiskImage:TypeOfDiskImage = TypeOfDiskImage.EMULATOR_FLOPPY_IMAGE):
        self.sectorSize = sectorSize = DiskSector.SIZE_OF_SECTOR_DD if typeOfDiskImage== TypeOfDiskImage.EMULATOR_FLOPPY_IMAGE else DiskSector.SIZE_OF_SECTOR_SDDRIVE
        self.trackSize = trackSize = sectorSize * SECTORS_PER_TRACK
        dataSize = len(rawData)
        if dataSize >0 and dataSize < trackSize:
            raise ValueError(f"Non empty rawData should have a length of {trackSize} bytes for {typeOfDiskImage.name}, got {len(rawData)}")
        
        if dataSize == 0:
            self._sectors = [DiskSector(typeOfDiskImage=typeOfDiskImage) for i in range(SECTORS_PER_TRACK)]
        else:
            self._sectors = [DiskSector(rawData[(i*sectorSize):(i*sectorSize+sectorSize)] for i in range(SECTORS_PER_TRACK))]
    ...
    def write(self, rawData:bytearray or bytes):
        if dataSize < trackSize:
            raise ValueError(f"rawData should have a length of {trackSize} bytes for {typeOfDiskImage.name}, got {len(rawData)}")

        for sector in self._sectors:
            sector.data = rawData[(i*sectorSize):(i*sectorSize+sectorSize)]
    
    def rawData(self):
        result = bytearray(self.trackSize)
        for index,sector in enumerate(self._sectors):
            result[index*self.sectorSize:(index+1)*self.sectorSize] = sector.data
        return bytes(result)
